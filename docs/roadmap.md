...
| 5 | Adaptador serie `EyeTransport` | Completado | `serial_adapter.py` y `tests/unit/hardware/test_serial_adapter.py` |
| 6 | Simulador conductual `FakeESP32` | Completado | ADR-0009/0010 y `tests/unit/hardware/test_fake_esp32.py` |
| 7 | Runtime con `asyncio`, registro de componentes, políticas y CLI | Completado | `app.py`, `cli/run.py` y `tests/unit/runtime/` |
...
| 9 | Laboratorio conversacional con VAD, STT, LLM, TTS y medición de latencia | Experimental validado en software y laboratorio | `sirah-conversation`, `tests/unit/conversation/`, `docs/conversation.md` y protocolo de laboratorio |
...
- La instalación base no exige OpenCV ni el modelo de YuNet (ADR-0006);
  `sirah-models yunet` los instala bajo demanda y `sirah-perceive` permite
  observar la cámara/detector sin armar ojos ni abrir el serial (ADR-0009).
