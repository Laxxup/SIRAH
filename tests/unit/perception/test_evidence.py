...
# ---------------------------------------------------------------------------
# Per-kind policy overrides (M8.1.1): gesture-only acquisition tuning
# ---------------------------------------------------------------------------


def test_hub_kind_override_widens_gesture_confirmation_only():
    hub = EvidenceHub(kind_overrides={"gesture": {"confirm_window_s": 1.5}})
    # two valid Victory observations 1.0 s apart: beyond the global 0.5 s
    # window, inside the gesture window -> confirms without multi-second delay
    hub.observe([_hub_raw("victory", 0.9, t=0.0)], now=0.0)
    snapshot = hub.observe([_hub_raw("victory", 0.9, t=1.0)], now=1.0)
    assert snapshot.state_values() == ("victory",)
    assert "gesture_victory_confirmed" in snapshot.event_values()


def test_hub_kind_override_does_not_change_face_person_policy():
    hub = EvidenceHub(kind_overrides={"gesture": {"confirm_window_s": 1.5}})
    hub.observe([_hub_raw("present", 0.9, t=0.0, source="yunet", kind="face", track="primary")], now=0.0)
    # face keeps the global 0.5 s window: 1.0 s apart resets the candidate
    snapshot = hub.observe(
        [_hub_raw("present", 0.9, t=1.0, source="yunet", kind="face", track="primary")],
        now=1.0,
    )
    assert "face_present_confirmed" not in snapshot.event_values()
    assert snapshot.state_values() == ()


def test_hub_kind_override_keeps_two_sample_confirmation():
    hub = EvidenceHub(kind_overrides={"gesture": {"confirm_window_s": 1.5}})
    # a single observation is never stable truth, even with a wide window
    snapshot = hub.observe([_hub_raw("victory", 0.9, t=0.0)], now=0.0)
    assert snapshot.state_values() == ()
    assert snapshot.pending and snapshot.pending[0].confirm_count == 1
    assert snapshot.pending[0].confirm_samples == 2
